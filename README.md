...
Cada cliente elige cómo recibe los resultados, en la ficha del cliente › Entrega de resultados. Hay una configuración general y, si hace falta, una propia por automatización; lo que venga en cada llamada a la API tiene prioridad (`callback_url` y `progreso` en `POST /v1/.../lote`, `callback_url` en una consulta suelta).

- **Enlace de seguimiento de los lotes** (activo por defecto, 7 días, datos enmascarados): `https://…/seguimiento/<token>` muestra a quien lo tenga, sin entrar a la consola, el avance, las cifras, las filas y la descarga en Excel o CSV. El token es aleatorio, vence y se puede regenerar o revocar desde el detalle del lote; la página se sirve con `noindex` y `no-referrer`. Con enmascarado, los documentos quedan como `****9789`, los nombres con iniciales y los contactos ocultos, también en el Excel del enlace.
- **Aviso al sistema del cliente:** POST JSON a una URL https (en local, también `http://localhost`), al terminar cada lote (`lote.terminado`, `lote.cancelado`) y, si se elige, en cada consulta (`consulta.completada`, `consulta.fallida`). Encabezados: `x-startia-evento`, `x-startia-id` (para no procesarlo dos veces), `x-startia-fecha` (segundos Unix) y `x-startia-firma: v1=<HMAC-SHA256 hex de "fecha.cuerpo">` con la clave de firma del cliente (`whsec_…`, visible y rotable en la ficha). Se rechazan URLs con usuario, direcciones internas y redirecciones.
- **Envío y reintentos:** los avisos se guardan en la colección `avisos` al cerrar consultas y lotes (`completar`, `fallar`, `actualizarLote`, `cancelarLote`). El worker llama a `POST /internal/avisos/despachar` en la API pública (`PUBLIC_URL`, por defecto la de Vercel) para que salgan desde la nube; si no responde, los envía desde el PC. Cada aviso se reclama de forma atómica y se intenta a los 0, 1, 4, 12 y 27 minutos; en la ficha y en el detalle del lote se ve su estado y se puede reintentar. Los avisos se borran a los 60 días.
- Las consultas procesadas por un worker sin actualizar no crean avisos por consulta; el cierre del lote se detecta cuando alguien consulta el lote (API, consola o enlace) y su aviso sale en ese momento.
